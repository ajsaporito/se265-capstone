...
<main id="contentContainer" class="flex-grow-1">
          ...
          <?php foreach ($requests as $request): ?>
            <div class="request-item mb-2">
              ...
              <?php if ($request['status'] === 'pending'): ?>
                <button class="btn btn-success accept-request-btn" data-request-id="<?= $request['request_id']; ?>" data-requested-by="<?= $request['requested_by']; ?>">
                  Accept
                </button>
              <?php endif; ?>
            </div>
  ...
  <script>
    $(document).on('click', '.accept-request-btn', function () {
      ...
      let requestedBy = $(this).data('requested-by');  // Get the correct user_id
